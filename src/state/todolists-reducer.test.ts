...
test('target todolist title must be changed', () => {
  ...
  const endState = todolistsReducer(startState, EditTodolistNameAC(todolistsId1, newTitle));
...
test('target todolist`s filter must be changed', () => {
  ...
  const endState = todolistsReducer(startState, ChangeTodolistFilterAC(todolistsId1, newFilter));
